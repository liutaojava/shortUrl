...
public class PECode {
	...
	public static String encode(Long number) {
		BigDecimal rest = BigDecimal.valueOf(number);

		// 创建栈
		Stack<Character> stack = new Stack<>();
		StringBuilder result = new StringBuilder(0);
		while (rest.compareTo(BigDecimal.ONE) >= 0) {
			// 进栈,
            BigDecimal[] bigDecimals = rest.divideAndRemainder(BigDecimal.valueOf(array.length));
			stack.add(array[bigDecimals[1].intValue()]);
			rest = bigDecimals[0];
		}
		for (; !stack.isEmpty();) {
			// 出栈
